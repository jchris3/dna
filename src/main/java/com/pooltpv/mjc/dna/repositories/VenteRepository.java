...
public interface VenteRepository extends JpaRepository<Vente,String> {
    @Query(value = "  select\n" +
            ...
            "        ANCICODE as code_intermediaire_dna,\n" +
            "        NVL(rc,0) as prime_nette_RC,\n" +
            "        NVL(droit_timbre,0) as dta \n"+
            " from \n"+
            "    ORASSADM.CIE_POOL_POL_RISQUE,intermediaire \n"+
            " WHERE \n" +
            "    DATESOUS BETWEEN :dateDebut AND :dateFin \n" +
            "    and INTERMEDIAIRE.CODEINTE = CODEINTESOUS \n" +
            "    AND CODE_CIE=:codeCompagnie"
            ,nativeQuery = true)
    List<Vente> findVentes(@Param("codeCompagnie") int codeCompagnie,@Param("codeAssureur") int codeAssureur, @Param("dateDebut") Date dateDebut, @Param("dateFin") Date dateFin);
}
