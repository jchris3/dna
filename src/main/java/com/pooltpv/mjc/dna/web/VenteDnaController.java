...
@RestController
@RequestMapping("/csv")
public class VenteDnaController {

    private VenteDnaService venteDnaService;
    ...
    public VenteDnaController(VenteDnaService venteDnaService) {
        this.venteDnaService = venteDnaService;
    }
    @RequestMapping(path = "/venteDna/{codeCompagnie}/{codeAssureur}/{dateDebut}/{dateFin}")
            ...
            if (dateValidatorUsingDateTimeFormatter.isDatediffValid(startDate, endDate) == true)
                //codeAssureurs.forEach(codeAssureur-> {
                    try {
                        ...
                        InputStreamResource file = new InputStreamResource(venteDnaService.load(venteDnaService.listVenteDnaDTO(codeCompagnie,codeAssureur,startDate, endDate)));
