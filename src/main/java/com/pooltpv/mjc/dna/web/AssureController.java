...
import com.pooltpv.mjc.dna.service.AssureService;
...
public class AssureController {
    ...
    @RequestMapping(path = "/assures/{dateDebut}/{dateFin}")
            ...
            if (dateValidatorUsingDateTimeFormatter.isDatediffValid(startDate, endDate) == true)
                while (itListCodeAssureurs.hasNext() && itlistCodeCompagniess.hasNext()) {
                    ...
                    try {
                        ...
                        assureService.writeAssureCsv(assureService.listAssureDTO(itlistCodeCompagniess.next(), itListCodeAssureurs.next(), startDate, endDate), response.getWriter());
    ...
    @GetMapping("/assures/{codeCompagnie}/{codeAssureur}/{dateDebut}/{dateFin}")
    public ResponseEntity<InputStreamResource> getFile(@PathVariable(name = "codeCompagnie")  int codeCompagnie,
        ...
        try {
           ...
           // for (int i=0;i<listCodeAssureurs.size();i++){
                String filename = "assure_" + codeAssureur+ "_" + LocalDate.now().format(DateTimeFormatter.ofPattern("dd-MM-yyyy")) + ".txt";

                InputStreamResource file = new InputStreamResource(assureService.load(assureService.listAssureDTO(codeCompagnie, codeAssureur, startDate, endDate)));
