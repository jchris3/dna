...
import com.pooltpv.mjc.dna.service.VehiculeDnaService;
...
@RestController
@RequestMapping("/csv")
public class VehiculeDnaController {

    private VehiculeDnaService vehiculeDnaService;
    ...
    public VehiculeDnaController(VehiculeDnaService vehiculeDnaService) {
        this.vehiculeDnaService = vehiculeDnaService;
    }
    @RequestMapping(path = "/vehiculesDna/{codeCompagnie}/{codeAssureur}/{dateDebut}/{dateFin}")
            ...
            if (dateValidatorUsingDateTimeFormatter.isDatediffValid(startDate, endDate) == true)
                //codeAssureurs.forEach(codeAssureur-> {
                    try {
                        ...
                        InputStreamResource file = new InputStreamResource(vehiculeDnaService.load(vehiculeDnaService.listVehiculeDnaDTO(codeCompagnie, startDate, endDate)));
